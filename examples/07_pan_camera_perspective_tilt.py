...
# --- Zoom --------------------------------------------------------------------
# Zoom multiplies the world-to-viewport scale: zoom>1 shows fewer world units
# in the source rect (so the world edges move off the source rect and the map
# fills more of the visible trapezoid). zoom<1 shows more world.
ZOOM_MIN = 0.3
ZOOM_MAX = 4.0
...
def _project_world_polygon(pts, cam_x, cam_y, zoom, a):
...
def render_perspective(context: dcg.Context, parent: dcg.DrawingList,
    ...
    for (pmin, pmax, fill, edge, thickness) in model.rects:
        ...
        proj = _project_world_polygon(corners, cam_x, cam_y, zoom, a)
        if proj is None:
            continue
    ...
    # Text -> project anchor, scale glyph size with depth and zoom
...
class PerspectivePanController:
    ...
    def set_zoom(self, sender, target, value) -> None:
        # Keep the avatar visually anchored: re-center camera on avatar after
        # zoom changes so the view doesn't lurch off-frame at extreme zooms.
    ...
    def _move(self, dx: float, dy: float) -> None:
        ...
        # Edge-band test runs in viewport-pixel space (after the zoom scale).
        # Camera adjustments are then converted back to world units by /zoom.
    ...
    def _clamp_cam_x(self, x: float) -> float:
        # Visible width in world units is VIEW_W / zoom. If that's larger than
        # the world, max_cam goes negative and we just clamp to 0.
