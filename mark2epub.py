...
def _mathml_for_latex(latex_expr):
    if not latex_to_mathml:
        return None
    try:
        return latex_to_mathml(latex_expr)
    except Exception:
        return None


def convert_latex_math_to_mathml(markdown_text):
    if not latex_to_mathml:
        return markdown_text

    def replace_display(match):
        latex_expr = match.group(1).strip()
        mathml = _mathml_for_latex(latex_expr)
        if not mathml:
            return match.group(0)
        return '\n<div class="math math-display">{}</div>\n'.format(mathml)

    with_display = DISPLAY_MATH_PATTERN.sub(replace_display, markdown_text)
    ...
    def replace_inline(match):
        ...
        if not mathml:
            return match.group(0)
        return '<span class="math math-inline">{}</span>'.format(mathml)

    return INLINE_MATH_PATTERN.sub(replace_inline, with_display)
...
def get_chapter_XML(md_filename,css_filenames):
    ...
    markdown_data = convert_latex_math_to_mathml(markdown_data)
...
if __name__ == "__main__":
    ...
    if not latex_to_mathml:
        print("Warning: latex2mathml is not installed. LaTeX math will remain unconverted.")
