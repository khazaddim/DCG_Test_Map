...
- [ ] 5.4.11.1 Fix stale statistics by clearing `OverlapDepthSorter.last_sort_stats` when `frame is None`; add a regression test for both selector settings that performs a frame-backed sort followed by a fallback sort on the same instance, verifies `last_sort_stats` and the fallback result's `sort_stats` are `None`, and preserves average-depth ordering
